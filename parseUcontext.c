...
unsigned int probeUCStack(ucontext_t m);
 ...
 */
unsigned int 
probeUCStack(ucontext_t m)
{
  //ucontext_t u;
  int gc = getcontext(&m);
  assert(!gc); //Did getcontext fail?!
  return *(int*)m.uc_stack.ss_sp;
